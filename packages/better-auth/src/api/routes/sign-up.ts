		...
		async (ctx) => {
			return runWithTransaction(ctx.context.adapter, async () => {
				...
				const shouldReturnGenericDuplicateResponse =
					...
					ctx.context.options.emailAndPassword.requireEmailVerification || ctx.context.options.emailAndPassword.callExistingUserCallback === true) && ctx.context.options.emailAndPassword.callExistingUserCallback !== false;
