...
<Callout>
  If you set `emailAndPassword.callExistingUserCallback` to `true` it will always call `emailAndPassword.onExistingUserSignUp` when their is a duplicate user.
  If you set it to `false` it will never call `emailAndPassword.onExistingUserSignUp`
</Callout>
